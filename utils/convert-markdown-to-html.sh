...
for infile in `find . -type f -name '*.md' | grep -v 'reveal\.js'`; do
    ...
    pandoc --template `git rev-parse --show-toplevel`/pandoc-template.html --standalone --metadata lang="en" --variable "pagetitle:$lineone" --css ${pathprefix}markdown.css\
           --from markdown --to html5 --output $outfile $infile
done

pandoc --template `git rev-parse --show-toplevel`/pandoc-template.html --standalone --metadata lang="en" --variable "pagetitle:Teaching Assistants" --css ../markdown.css --css tas.css\
