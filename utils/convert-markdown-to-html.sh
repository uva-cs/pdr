...
for infile in `find . -type f -name '*.md' | grep -v 'reveal\.js'`; do
    ...
    pandoc --standalone --metadata lang="en" --variable "pagetitle:$lineone" --css ${pathprefix}markdown.css\
           --from markdown --to html5 --output $outfile $infile
done

pandoc --standalone --metadata lang="en" --variable "pagetitle:Teaching Assistants" --css ../markdown.css --css tas.css\
